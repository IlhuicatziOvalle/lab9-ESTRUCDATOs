...
int compararSeveridad(void *a, void *b) {
    ...
    Pasajero *p1 = (Pasajero *)a;
    Pasajero *p2 = (Pasajero *)b;

    // Comparar las severidades
    if (p1->informacion->prioridad < p2->informacion->prioridad) {
        return 1;  // p1 es menor
    } else if (p1->informacion->prioridad > p2->informacion->prioridad) {
